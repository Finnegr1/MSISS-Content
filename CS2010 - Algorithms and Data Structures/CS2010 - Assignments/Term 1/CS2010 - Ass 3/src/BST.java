...
public class BST<Key extends Comparable<Key>, Value> {
     ...
     * worst case O(N) which would describe a case where the tree only has left/right nodes
    ...
    private Key median(int k) {                  
        if (k < 0 || k >= size()) return null;
        Node x = median(root, k);
        return x.key;
    }
    ...
    private String prettyPrintKeys(Node a,String prefix)
    {
    	if (a==null)
    	{
    		return  prefix+"-null\n";  
    		...
    		return (prefix+"-"+a.key.toString()+"\n" +prettyPrintKeys(a.left,(prefix+" |"))+ prettyPrintKeys(a.right,(prefix+"  ")));
    	}
